---
title: "04 Spatial Interaction Analysis"
...
The graph file (`hamilton_graph_zoned.RData`) will look familiar to you from Reading and Exercise 03 ("Network Analysis"). As the file you used before, this one includes a portion of the road network in Hamilton, sourced from Open Street Maps. The difference with the network you used before is that it now has all roads, including provincial highways (e.g., the 403), and some nodes are labelled according to the zoning system used for the analysis of transportation data in Hamilton (see next). 

The file `taz_hamilton.RData` includes the boundaries of the _traffic analysis zones_ for Hamilton (or _taz_). Traffic analysis zones are simply a zoning system used for collecting and analyzing transportation data. The zoning system for Hamilton was designed and is used by the [Transportation Tomorrow Survey](http://www.transportationtomorrow.on.ca/), the periodic travel survey conducted every five years in the Metropolitan Regions of Toronto and Hamilton since 1986. The zoning system is a simple features object suitable for mapping and data analysis in `R`. The source shape file can be obtained [here](http://dmg.utoronto.ca/survey-boundary-files). In addition to the zoning system, the file has been augmented with information from the Transportation Tomorrow Survey, including (for each zone) total population, number of dwellings by type, number of households by income level, number of drivers, vehicles, and full and part-time employees, as well as total jobs in each zone by type of job. While these data are available publicly, permission to download them directly needs to be obtained - click this [link](http://dmg.utoronto.ca/drs-access) if you are interested in learning more about how to access these data directly. The zonal identifiers have been linked to the nodes in the road network that are closest to the centroid of the zone. In this way, we can relate the zones to the road network.

The last file that you will need for this notebook is `od_full.RData`. This file contains an _origin-destination_ table. An origin destination table superficially looks similar to a table of _links_ in a network: it has identifiers for origins (from) and destinations (to), coordinates for the origins and destinations, a unique identifier for the origin-destination pair, and importantly the number of trips between the origin and destination. This is in fact a network, but unlike the road network in `hamilton_graph_zoned`, it is not a physical one; instead, it is a point-to-point notional network with all the possible origins connected to all possible destinations by means of the number of trips (in many cases zero, since a vast number of zones do not actually have trips between them!). The origin-destination table, like the traffic analysis zones, was sourced from the Transportation Tomorrow Survey, and corresponds to the year 2016.
...
The fundamental idea is an analogy to Newton's Law of Universal Gravitation, originally published in Philosophiæ Naturalis Principia Mathematica in 1687. Carey's insight that similar principles as in physics could be at work in human interaction did not receive widespread attention, and the idea was only partially used by Ravenstein (1889) for the study of migration.
...
We will next load and inspect the data that will be used in this reading. Before starting, however, it is useful to make sure that we have a clean Environment. The following command will remove all data and values currently in the Environment.
```{r}
rm(list = ls())
```

Next, we must load the libraries needed for analysis (some may need to be installed before they are available for loading):
```{r message=FALSE}
library(igraph)
library(scales)
library(sf)
...
Load traffic analysis zones (taz):
```{r}
...
The three chunks above read the data files. We will now proceed to inspect the data objects that we loaded. We will begin with the object `hamilton_graph_zoned`:
```{r}
hamilton_graph_zoned
```

You worked with a road network in `tbl_graph` format in Reading and Exercise 3, where you conducted various types of network analysis. Recall that objects of class `tbl_graph` are also of class `igraph` which allows us to use package `igraph` to conduct many different types of network analyses - including finding shortest paths. The two main elements of a `tbl_graph` object are tables for nodes and edges. Each of these elements can be _activated_ using the function `tidygraph::activate()` as needed.

Notice that, compared to the network that you used before, the nodes include a new column, called `GTA06`. This column contains the unique identifier for a zone, and there is only one node in each zone that has this identifier. More on this below.

The next object that we will inspect is the zoning system. The object `taz_hamilton` is a simple features object with the boundaries of the traffic analysis zones in Hamilton. Traffic analysis zones are used to collect and report travel surveys. Together with the road network, the traffic analysis zones provide the necessary geographical information to study spatial interaction. 

The following chunk plots the road network and the traffic analysis zones:
```{r}
ggplot() + # Create blank ggplot object
  geom_sf(data = hamilton_graph_zoned %>% # To plot a `sf` object we use `geom_sf()`, and for this plot we want to use `hamilton_graph_local`
  ...
  geom_sf(data = taz_hamilton, # Plot the `sf` object; here we want to use `taz_hamilton`
...
Presently, the geometric centroid of the zones is considered, and there is a one-to-one identifier `GTA06` between the nodes in the network that are nearest to the centroid of a zone, and the identifier of the zone.

The next chunk plots the road network and the traffic analysis zones; in this map, centroid nodes are red diamonds:
```{r}
ggplot() + # Create blank ggplot object
  geom_sf(data = hamilton_graph_zoned %>% # To plot a `sf` object we use `geom_sf()`, and for this plot we want to use `hamilton_graph_local`
            activate(edges) %>%  # Activate the edges of the `tbl_graph` object; After activating the edges, the object will behave as an `igraph` or network object
...
The `taz_hamilton` file contains, in addition to the geometry of the boundaries, socio-economic and demographic information about the zones:
```{r}
summary(taz_hamilton)
```
The columns starting with `Pi_` are the trips (work or shopping) that are _produced_ by the zone: in other words, the number of trips that start in that zone. The columns starting with `Pj_` are the trips (work or shopping) that are _attracted_ by the zone: in other words, the number of trips that end in that zone. The table also includes the number of dwellings of three different types: houses, apartments, and townhouses. There is a column with the total population in the zone. Columns that begin with `Inc_` are the number of households in the zone at different levels of income (i.e., less than CAD15,000, between CAD15,000 and CAD40,000, and so on). There is also the total number of people in the zone who are licenced to drive, the number of people with full and part-time jobs whose residence is in the zone, and the total number of vehicles owned by households in the zone. The last few columns that begin with `Jobs_` are the number of jobs in different classes in the zone, and measure the employment available in the zone.

You can explore these variables by means of summary statistics:
```{r}
summary(taz_hamilton)
```

For example, the maximum number of work trips produced by any zone (`Pi_work`) is 2,367, and the maximum number of shopping trips attracted by any zone (`Pj_shop`) is 8,008. There are some NAs in the table (missing values). These missing values are the result of some zones being purely residential (therefore there is no employment there) or some zones being purely industrial or commercial, with no residential population.

In addition to summary statistics, you can explore these variables by means of thematic maps; here, the map is of `Pi_work`, the number of work trips produced by zones:
```{r}
ggplot() + # Create a blank ggplot object
  geom_sf(data = taz_hamilton, # Plot the `sf` object; here we want to use `taz_hamilton`
          aes(fill = Pi_work)) + # Make the fill color of the polygons a function of the number of work trips produced by zones
...
Which zones produce large numbers of work trips? Do you notice a pattern?

We will create a variable called `total_employment`, as the sum of all types of jobs in a zone:
```{r}
taz_hamilton <- taz_hamilton %>%
  mutate(total_employment = Jobs_Office_Clerical + Jobs_Manufacturing_Construction_Trades + Jobs_Professional + Jobs_Retail)
```

The next object is `od_full`. This is also a simple features object, with lines showing the connections between zone centroids. We can see the first few rows at the head of the table:
```{r}
head(od_full)
```

The table includes columns with zone identifiers for the origin and destination zones. The identifiers are the same as the columns `GTA06` in the nodes of the road network and the traffic analysis zones. This table is very special: although information about trips was in the `taz_hamilton` object (i.e., the `Pi_` and `P_j` columns), there the trips were total produced and attracted by the zones. The origin-destination table gives more detailed information about the trips by disaggregating those totals by origin zone and destination zone! This is much richer information than just the totals and constitutes the core of spatial interaction analysis: it tells us precisely (within the context of the zoning system) where do trips begin and where they end.

We see, for example, that zone 5002 did not _interact_ with zone 5001 (the first row in the table), since the number of work and shopping trips originating in zone 5002 and ending in zone 5001 was zero. In contrast, we see that zone 5007 _interacted_ with zone 5001, since there were 19 shopping trips with 5007 as the origin and 5001 as the destination.

We can verify that the trip information in the tables `taz_hamilton` and `od_full` is consistent by summarizing the trips in the origin-destination table. :
```{r}
...
The corresponding information in the `taz_hamilton` table is:
```{r}
taz_hamilton %>% # Pass the object `od_full` to the next function
  st_drop_geometry() %>% # Drop the geometry of the object
  filter(GTA06 == 5002) %>% # Filter one zone to inspect
  select(Pi_work, Pi_shop) # Select the columns with trip totals by zone of origin
```
...
This plot is essentially a point-to-point network, but it is not very informative. For one reason, the trips do not actually move in _straight lines_ from origin to destination, but rather move on the road network. This kind of plot is sometimes called _desire lines_. Another reason the plot is not very informative is that a lot of the lines are actually for zone pairs with zero trips! The next chunk improves the plot in three ways: it includes the zoning system to provide more geographical context, it removes the lines that are for small number of trips, and modifies the lines (by changing their color, thickness, transparency) to enhance legibility:
```{r}
ggplot() + # Create a blank ggplot object
  geom_sf(data = taz_hamilton, # Plot the `sf` object; here we want to use `taz_hamilton`
          size = 1, # Change the thickness of the zone boundaries for ease of visualization
...
To illustrate the analysis of spatial interaction we need to prepare some additional information. The next item that we need is the separation between origins and destinations. While this information is not included in the data provided, you already have the tools (from Exercise 3) to calculate the shortest path distances between origins and destinations.

To calculate the shortest paths we need to add the length of the links to the information that we have for the network:
```{r}
...
The shortest-path cost can be calculated by means of the `igraph` function `distances()`; the difference with the way you used this in Exercise 3 is that we do not want the shortest paths between _every_ origin and destination pair, but only between centroid nodes:
```{r}
centroid_distances <- distances(hamilton_graph_zoned, # Calculate shortest path costs in a graph
...
The longest network distance along a shortest path between two zones in Hamilton is 52.441 km. This is the diameter of the network according to length.

Note that we could use a different metric for the separation between the zones, for instance time or cost. Time, in particular, is related to distance by speed:
...
We can use `mutate()` to add a new column for travel time (see line 445 in `03 Network Analysis.Rmd`). While in Reading 3 we assumed that speed was constant for all link types, here we will assume that speed is different for different types of links. In this case, we will use the function `case_when()` to assign speeds to various cases:
```{r}
...
Use time to find the shortest path travel time between zone centroids:
```{r}
...
The diameter of the network in Hamilton in travel time is 0.573 h, or 34.4 minutes, approximately.
...
centroid_costs <- expand.grid(Origin = hamilton_graph_zoned %>% # The function `expand.grid()` creates a table with all the combinations of values given in the inputs; the first input is the vector of zone identifiers of the origin nodes in the network used to calculate the distances
  ...
  mutate(network_distance = centroid_distances %>% # Use `mutate()` to add a column with the inter-centroid shortest path distances 
           ...
           set_units("km"), # Set units to kilometers
...
The final data pre-processing step is to join the attributes of the zones to the origin-destination table. To do this, we need to think about which attributes are of interest at the origin and which at the destination. In the current example, there are attributes that we think contribute to _producing_ trips at the origin: these are population, number of dwellings, number of households, number of drivers, number of workers, and number of vehicles. Then, there are attributes that we think contribute to _attracting_ trips at the destination, such as employment variables. All these variables are currently in the `taz_hamilton` object, so we wish to copy them to the origin-destination table. Next, we join the attributes that correspond to the origins:
```{r}
...
od_full <- od_full %>% # Pass object `od_full` to the next function
  left_join(taz_hamilton %>% # Join to table `taz_hamilton`
              select(GTA06, Pj_work:total_employment) %>% # Select from table `taz_hamilton` columns `GTA06` (the zone identifier), and then those ranging from `Pj_work` to `total_employment`
              st_drop_geometry(), # Drop the geometry of `taz_hamilton`
...
```{r}
pop_deciles <- quantile(taz_hamilton$population, # Find the quantiles of variable `population` in table `taz_hamilton`
...
emp_deciles <- quantile(taz_hamilton$total_employment, # Find the quantiles of variable `total_employment` in table `taz_hamilton`
                        seq(0, 1, 0.1), # The quantiles will be a sequence of values between 0 and 1, in 0.1 intervals, i.e., the deciles 
                        na.rm = TRUE) # Remove NAs when calculating the deciles
...
The way to read the plot above is as follows: the ten percent of zones with the smallest populations (between 8 and 219 people) account for a tiny proportion of the population in Hamilton (about 0.03%), but produce approximately 0.3% of all work trips; the next ten percent of zones (with populations between 219 and 607), contain almost ten times the proportion of the population (0.25%) but produce 1.7% of all work trips (or about six times more than the proportion of the bottom ten percent of zones). The number of trips for the smaller zones grows more rapidly than the population. But the effect is not linear: bigger zones do produce more trips, but not proportionally more than their share of the population. This is noticeable in how the green columns (frequency of trips) are initially taller than the red columns (frequency of population), but then the red columns are bigger than the green. The top ten percent of the largest zones in Hamilton (with populations between 5,050 and 8,760 people) account for approximately 39% of the population of the city, but only about 25% of all trips.
...
The non-linear effect of mass at the destination is even more marked than the effect for mass at the origin in the previous figure: notice how the top ten percent of zones with the largest number of jobs (between 1,710 and 9,390 jobs) account for close to 80% of all jobs in Hamilton, and attract just under 50% of all work trips in the city. This is indicative of the large concentration of employment in a relatively small number of zones in the city. This spatial organization of employment of course can have a large impact on commuting patterns.
...
Calibration of the gravity model uses regression analysis (see Reading `02 Simple linear regression in R`). There are two differences with the way you used regression analysis in Exercise 2:
...
$$
T_{ij} \propto Pop^{0.179777}
$$

We can visualize this effect by simulating a population variable ranging from 10 (approximately the population of the least populous zone in Hamilton) to 8,770 (approximately the population of the most populous zone in Hamilton):
```{r}
data.frame(population = seq(from = 10, # Simulate a population variable
                            to = 8770,
...
The mass effect of population at the origin increases rapidly for small populations, but then more slowly for large populations; in other words, larger populations do not produce proportionally more trips. For example, the smallest population in the plot (10 people) tends to increase the number of work trips by a factor of 1.5. In contrast, the largest population in the plot (8,770 people, that is 877 times more people than the smallest zone) tends to increase the number of work trips by a factor of only 5.115 (or only three times more than the smallest population).

We can also visualize the cost effect, which essentially means plotting the distance-decay function:

$$
f(c_{ij}) = c_{ij}^{-0.1114643} = \frac{1}{c_{ij}^{0.1114643}}
$$
...
```{r}
data.frame(distance = seq(0.1, # Simulate a distance variable
                          41, 
                          0.1)) %>% # Use 0.1 km (i.e., 100 m) increments
...
We see that the potential for interaction declines quite rapidly for distances between 100 m and 5 km, and then more slowly after that. The effect on spatial interaction is as follows: other things being equal, at short distances the number of trips is somewhat higher than proportional. But after distances of greater than 1 km (when the cost effect is essentially neutral, since 1 raised to any power is 1), the effect of greater distances is to reduce the potential for interaction to fractions of what it would otherwise be (for example, at a distance of approximately 25 km, the number of trips is reduced by a factor of 0.7).
...
The total number of work trips in the table is 110,243 and the model predicts 110,243 trips (it is a property of the Poisson model that if it includes a constant the total of the predicted values matches the total of the observed values).
...
The calibrated gravity model is informative about the trip-making behavior of the public. Particularly, the distance-decay function tells us about how people respond to the friction of space. This information can be used in different ways. Here, we will illustrate how it is useful to calculate accessibility.

Recall that accessibility is defined as the weighted sum of opportunities from the perspective of an origin:

$$
\underbrace{A_i}_\text{Accessibility}=\overbrace{\sum_{j=1}^N}^\text{Sum of all opportunities}\underbrace{W_j}_{\text{Opportunity j}}\overbrace{f(\underbrace{c_{ij})}_\text{cost of travel between i and j}}^\text{decay function}
$$

The calibrated distance-decay function can be used to estimate the accessibility. In this example, we will calculate accessibility to jobs, and the measure of opportunities will be total employment, which is in the origin-destination table `od_full`.
...
Accessibility in Hamilton ranges from 110,095 jobs to 135,008 jobs. We can join the accessibility to the zoning system to map this result:
```{r}
taz_hamilton %>% # Pass object `taz_hamilton` to the following function
  left_join(accessibility_to_jobs, # Join table `accessibility_to_jobs` to `taz_hamilton`
            by = c("GTA06" = "Origin")) %>% # The keys for the join are "GTA06" (the taz identifier) on `taz_hamilton` and "Origin" (the taz identifier) on `accessibility_to_jobs`; pass to ggplot
